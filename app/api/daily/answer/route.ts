...
try { getSupabaseAdmin = require("@/lib/supabase-admin").getSupabaseAdmin; } catch { getSupabaseAdmin = null; }

type EV = "E" | "V" | "Λ" | "Ǝ";
type Theme = "WORK" | "LOVE" | "FUTURE" | "LIFE";
type DailyAnswerRequest = { seed?: number; choiceId?: string; slot?: "morning"|"noon"|"night"; theme?: Theme; };
type DailyAnswerResponse =
  | { ok:true; code:EV; score:number; comment:string; advice:string; affirm:string; env?:"dev"|"prod" }
...
function scoreFromSeed(seed?: number, bias=0) {
...
const AFFIRM: Record<EV,string> = {
  E:"私は小さく速く動ける。", V:"私は可能性を言葉にできる。", Λ:"私は迷いなく選べる。", Ǝ:"私は静けさから整えられる。"
};
...
export async function POST(req: NextRequest) {
  try {
    ...
    const res: DailyAnswerResponse = { ok:true, code:ev, score, comment, advice, affirm, env:"prod" };
