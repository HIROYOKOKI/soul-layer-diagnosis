...
/* =============================================================
   ...
   - 全て named export（default export なし）
...
export const DEFAULT_COLORS: Readonly<EVColors> = {
  ...
  V: '#1E3A8A',
  L: '#84CC16',
  Eexists: '#B833F5', // Ǝ vivid purple（大文字統一）
}

export const DEFAULT_ORDER: Readonly<EVKey[]> = KEYS

export function labelOf(k: EVKey) {
  return k === 'L' ? 'Λ' : k === 'Eexists' ? 'Ǝ' : k
}
export function clamp01(n: unknown) {
...
export function EVAEColorBadges({
  values, colors = DEFAULT_COLORS, size = 24, box = 40, rounded = 10, gap = 12,
  highlightMax = true, showScore = false,
}: {
  values?: Partial<EVVals>; colors?: Readonly<EVColors>; size?: number; box?: number;
  rounded?: number; gap?: number; highlightMax?: boolean; showScore?: boolean
}) {
  const v: EVVals = {
          ...
          <div key={k} className="relative flex flex-col items-center">
            <div
              ...
              style={{
                ...
                boxShadow: isMax
                  ? `0 0 60px ${color}FF, 0 0 30px ${color}CC, inset 0 0 28px ${color}AA`
                  : `inset 0 0 18px ${color}55`,
...
export function EVAEPolarChart({
  ...
  const pts = KEYS.map((k,i) => {
    ...
    return [half + Math.cos(angles[i]) * r(v), half + Math.sin(angles[i]) * r(v)] as const
  })
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {gridCircles.map((rr,idx)=> (
        <circle key={idx} cx={half} cy={half} r={rr} fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth={1} />
      ))}
