...
export function RadarChart({ values, size = 260 }: { values: EVAEVector; size?: number }) {
  ...
  const polar = (deg:number, s:number) => {
    const rad = (Math.PI/180)*deg, rr = r*clamp01(s)
  ...
  const colorFor = (l:"E"|"V"|"Λ"|"Ǝ") => l==="E"?PALETTE.E : l==="V"?PALETTE.V : l==="Λ"?PALETTE.L : PALETTE.Eexists
        ...
        {axes.map(a=>{
          ...
          return (
            <g key={a.label}>
              ...
              <text x={x} y={y} dy={a.angle===90?12:a.angle===-90?-6:4} fontSize={13} fontWeight={700} fill={c}>{a.label}</text>
            </g>
          )
        })}
        <defs>
          ...
          <filter id={`blur-${gid}`} x="-50%" y="-50%" width="200%" height="200%"><feGaussianBlur in="SourceGraphic" stdDeviation="6"/></filter>
        </defs>
