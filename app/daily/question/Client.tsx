...
const EV_COLOR: Record<EV, string> = {
  E: "text-orange-400", V: "text-indigo-300", "Λ": "text-emerald-300", "Ǝ": "text-fuchsia-400",
}
...
export default function DailyQuestionClient() {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      let next = idx
      if (["ArrowRight","ArrowDown"].includes(e.key)) next = Math.min(3, (idx + 1 + 4) % 4)
      if (["ArrowLeft","ArrowUp"].includes(e.key)) next = Math.max(0, (idx - 1 + 4) % 4)
      if (next !== idx && next >= 0) { e.preventDefault(); choose(keys[next]) }
      if ((e.key === "Enter" || e.key === " ") && finalK) { e.preventDefault(); onSubmit() }
    }
  ...
  if (loading) return <div className="min-h-screen bg-black text-white grid place-items-center">読み込み中…</div>
  if (err && !q) return (
    <div className="min-h-screen bg-black text-white grid place-items-center">
      <div className="text-center space-y-3">
        <div className="text-red-300">読み込みに失敗しました</div>
        <button className="rounded-xl border border-white/20 px-4 py-2" onClick={() => location.reload()}>再読み込み</button>
      </div>
    </div>
  )
          ...
          <section className="rounded-2xl border border-white/12 bg-white/5 p-5 space-y-5">
              ...
              {q.options.map(o => {
                ...
                return (
                  ...
                  >
                    ...
                    {firstPick && !active && (
                      <div className="mt-1 text-[11px] text-white/60">最初に選択</div>
                    )}
                  </button>
                )
              })}
            ...
            {err && (
              <div className="rounded-md border border-red-400/40 bg-red-400/10 px-3 py-2 text-sm text-red-200">
                送信できませんでした。あとで再送します。
                <button onClick={flushQueue} className="ml-2 underline">今すぐ再送</button>
              </div>
            )}
          </section>
        )}
        ...
        {done && (
          <section className="rounded-2xl border border-white/12 bg-white/5 p-5 space-y-3">
            ...
            <div className="pt-2 flex gap-2">
              <a href="/mypage" className="flex-1 rounded-xl border border-white/20 px-4 py-2 text-center hover:bg-white/10">マイページへ</a>
              <button onClick={() => location.reload()} className="rounded-xl border border-white/20 px-4 py-2 hover:bg-white/10">もう一度</button>
            </div>
          </section>
        )}
      </main>
