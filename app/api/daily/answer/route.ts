...
// Supabase が設定されていれば保存に使う（無ければスキップ）
let getSupabaseAdmin: (() => any) | null = null;
try {
  // ある環境では無い可能性があるため try-catch で安全に
...
export async function POST(req: NextRequest) {
  try {
    ...
    // theme を決定（body > query > cookie > LIFE）
    ...
    // ===== Optional: Supabase 保存（存在すれば） =====
    if (getSupabaseAdmin) {
      try {
        const sb = getSupabaseAdmin!();
        if (sb) {
          await sb.from("daily_results").insert({
            // user_id は middleware/セッションによっては取れないので省略（後で拡張）
            slot: body?.slot ?? null,
