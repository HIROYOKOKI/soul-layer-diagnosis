...
export async function GET() {
  ...
  // base_order_v2(jsonb) を優先、旧 base_order(text/array) をフォールバック
  ...
  const item = {
    ...
    base_model: (data.base_model === "EΛVƎ" || data.base_model === "EVΛƎ" ? data.base_model : null) as Model | null,
    base_order: normOrder((data as any).base_order_v2 ?? (data as any).base_order), // ← ここで EV[] に統一
