...
export default function ThemeClient() {
  ...
  const handleSelect = (k: ThemeKey) => {
    setSelected(k);
    if (typeof window !== "undefined") sessionStorage.setItem("evae_theme_selected", k);
    // 触感フィードバック（対応端末のみ）
      ...
      <div className="fixed inset-x-0 bottom-0 bg-gradient-to-t from-black/85 to-transparent px-5 pb-[max(16px,env(safe-area-inset-bottom))] pt-3 border-t border-white/10">
        <button
          ...
          className={[
            ...
            !selected ? "bg-white/10 text-white/50" : "bg-white text-black active:opacity-90",
          ].join(" ")}
        >
          確認へ進む
        </button>
