...
export const LUNEA: Character = {
  ...
  messages: {
    ...
    quoteByCode: {
      ...
      V: "Imagination is everything. — Albert Einstein",
      Λ: "道を選ぶのは、あなた自身だ。— 老子（意訳）",
      Ǝ: "Somewhere, something incredible is waiting to be known. — Carl Sagan",
    },
  },
  runtime: {
...
function ensureNamePrefix(text: string, name?: string | null): string {
  ...
  if (!n) return text;
  const prefix = `${n}さん、`;
  const headTrimmed = text.replace(/^[\s\u3000]+/, "");
  return headTrimmed.startsWith(prefix) ? text : `${prefix}${text}`;
}

export type LuneaSpeechKind = "start" | "beforeQuestion" | "afterResult" | "closing";

/** 各画面ごとのセリフ取得（名前必須運用・先頭に付与） */
export function luneaSpeech(
  kind: LuneaSpeechKind,
  code?: StructureCode,
...
export function luneaMypageClosing(
  latestCode?: StructureCode,
  userName?: string | null
): string {
  const mode: LuneaMode = "friend";
...
export const LUNEA_SYSTEM_PROMPT = `あなたは「${LUNEA.persona.displayName}」。日本語で簡潔に、あたたかく、断定しすぎないトーンで話します。
...
export function applyLuneaTone(
  text: string,
  _mode: LuneaMode = DEFAULT_LUNEA_MODE
): string {
  return applyFriendTone(text);
}
