      ...
      {!p ? (
        ...
        <>
          <ul className="text-sm grid gap-1">
            <li>名前：{p.name}</li><li>生年月日：{p.birthday}</li>
            <li>血液型：{p.blood}</li><li>性別：{p.gender}</li><li>恋愛対象：{p.preference}</li>
          </ul>
          {error && <p className="text-red-400 text-sm">エラー：{error}</p>}
          <div className="flex gap-8">
            <button onClick={goBack} className="underline">修正する</button>
            <button onClick={runDiagnose} disabled={loading} className="rounded bg白/10 px-4 py-2 disabled:opacity-50">{loading ? '診断中…' : '診断する'}</button>
          </div>
        </>
      )}
    </div>
