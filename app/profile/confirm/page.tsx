...
export default function ProfileConfirmPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setP(JSON.parse(raw) as Pending)
    } catch {}
  }, [])
  ...
  const runDiagnose = async () => {
    ...
    try {
      const res1 = await fetch('/api/profile/diagnose', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(p)
      })
      const j1 = await res1.json() as DiagnoseResp
      if (!j1.ok) throw new Error(j1.error)

      const res2 = await fetch('/api/profile/save', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...p, ...j1 })
      })
    ...
    <div className="mx-auto max-w-md grid gap-4 p-4">
      ...
      {!p ? (
        <div className="text-sm opacity-75">
          入力内容が見つかりません。<a href="/profile" className="underline">プロフィールに戻る</a>
        </div>
      ) : (
        <>
