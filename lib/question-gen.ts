...
/** 直近N件の偏りから「迷いそうな2〜3択」を抽出（少数派を優先） */
...
/**
 * 入力データから質問を構築（同期・純関数）
 * - rows/theme は呼び出し元で収集（RLSに従う）
 * - subset を付与し、options を 2〜3択へ縮約する場合あり
 */
export function buildQuestionFromData(
  userId: string, // 署名用（今は未使用。将来の個別メッセージに備えて残す）
  slot: Slot,
