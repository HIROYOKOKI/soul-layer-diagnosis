...
export default function EmailSettingsClient() {
  ...
  useEffect(() => {
    ...
    if (status === "confirmed") setMsg("メールアドレスの変更が完了しました。");
  }, [sp]);
  ...
  async function onSubmitChangeEmail(e: React.FormEvent) {
    ...
    try {
      ...
      if (error) {
        if (/reauth|re-auth|requires reauthentication|confirm your identity/i.test(error.message || "")) {
          setNeedReauth(true);
  ...
  async function onReauth(e: React.FormEvent) {
    ...
    setBusy(true); setMsg(null); setErr(null);
    try {
      const { error } = await sb.auth.signInWithPassword({ email: reauthEmail, password: reauthPwd });
  ...
  return (
    <div className="mx-auto max-w-md px-6 py-10">
      ...
      <form onSubmit={onSubmitChangeEmail} className="space-y-4">
        ...
        <input
          type="email" required placeholder="new@example.com"
      ...
      {needReauth && (
        <div className="mt-8 border rounded-md p-4">
          ...
          <form onSubmit={onReauth} className="space-y-3">
            <input type="email" value={reauthEmail} onChange={(e) => setReauthEmail(e.target.value)}
              className="w-full rounded-md border px-3 py-2 bg-transparent" placeholder="you@example.com" autoComplete="email" />
            <input type="password" value={reauthPwd} onChange={(e) => setReauthPwd(e.target.value)}
              className="w-full rounded-md border px-3 py-2 bg-transparent" placeholder="パスワード" autoComplete="current-password" />
            <div className="flex gap-8">
              <button type="submit" disabled={busy}
                className="flex-1 rounded-md bg-white text-black py-2 font-medium disabled:opacity-50">
                {busy ? "確認中…" : "パスワードで再認証"}
              </button>
              <button type="button" onClick={onReauthByMagicLink} disabled={busy}
