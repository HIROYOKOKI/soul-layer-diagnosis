    ...
    <main style={styles.page}>
      ...
      <section style={styles.card}>
        ...
        <div style={styles.row}>
          <DomeButton label="はじめて" variant="pink" onClick={() => alert('オンボーディング（仮）')} />
          <DomeButton label="ログイン" variant="blue" href="/login/form" />
        </div>
      </section>

      {/* 背景：オーラ＋星屑（背面固定・クリック奪わない） */}
...
function DomeButton({
  ...
  return (
    ...
    >
      {href ? (
        <Link href={href} style={baseStyle}>{core}</Link>
      ) : (
        <button type="button" onClick={onClick} style={baseStyle}>{core}</button>
      )}
    </div>
  )
}
...
const styles: Record<string, CSSProperties> = {
  ...
  subtitle: { margin:'2px 0 8px', opacity:.8 },
  ...
  aura: {
    position:'absolute', inset:'auto', left:'50%', top:'48%',
    width:420, height:420, transform:'translate(-50%, -50%)',
    borderRadius:'50%',
    background:'radial-gradient(circle, rgba(79,195,255,.35), rgba(0,0,0,0) 60%)',
    filter:'blur(22px)',
    animation:'pulse 5s ease-in-out infinite',
  },
  aura2: {
    position:'absolute', left:'58%', top:'28%',
    width:260, height:260, borderRadius:'50%',
    background:'radial-gradient(circle, rgba(255,79,223,.28), rgba(0,0,0,0) 60%)',
    filter:'blur(18px)', animation:'drift 7s ease-in-out infinite',
  },
  noise: {
