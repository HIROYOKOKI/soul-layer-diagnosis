...
/* =============================================================
   ...
   - Ǝ vivid purple color normalized to '#B833F5'
   ============================================================= */
...
const DEFAULT_COLORS: Readonly<EVColors> = {
  E: '#FF4500',
  V: '#1E3A8A',
  L: '#84CC16',
  Eexists: '#B833F5', // Ǝ vivid purple (uppercase normalized)
}

const DEFAULT_ORDER: Readonly<EVKey[]> = KEYS
...
function hexToRGBA(hex: string, a: number) { const h = hex.replace('#',''); const n = parseInt(h.length===3 ? h.split('').map(c=>c+c).join('') : h, 16); const r=(n>>16)&255,g=(n>>8)&255,b=n&255; return `rgba(${r},${g},${b},${a})` }

/* 追加：最大スコアのキーを返す */
function maxKeyOf(values: EVVals): EVKey {
      ...
      {ord.map((k: EVKey)=> (
        <div key={k} className="flex items-center gap-1">
          <span aria-label={labelOf(k)} className="inline-block" style={{
            ...
            background: colors[k],
            boxShadow: glow? `0 0 32px ${colors[k]}EE` : undefined,
...
/* ===================== Trend (dummy) ===================== */
export function EVAETrendChart({ days = 90, windowSize = 30, colors = DEFAULT_COLORS }: { days?: number; windowSize?: 7|30|90; colors?: Readonly<EVColors> }) {
  const pad = 24, logicalW = 720, h = 260
  const innerW = logicalW - pad*2, innerH = h - pad*2
  const mk = (seed:number)=> (i:number)=> { const t=(i+seed)/6; const base=(Math.sin(t)+1)/2, wobble=(Math.sin(t*0.7+seed)+1)/4, noise=(Math.sin(i*1.37+seed*2)+1)/10; return Math.max(0, Math.min(1, base*0.6 + wobble*0.3 + noise*0.2)) }
