//api/daily/question も EVLA 依存なし版に上書き
cat > app/api/daily/question/route.ts <<'TS'
...
async function getUserName() {
  ...
  if (!user) return null;
  const { data: prof } = await sb.from("profiles").select("name,display_id,user_no").eq("id", user.id).maybeSingle();
  return (prof?.name || prof?.display_id || prof?.user_no || user.email?.split("@")[0] || null) as string | null;
}
...
function getJstSlot(): Slot {
  try {
    const parts = new Intl.DateTimeFormat("ja-JP",{ timeZone:"Asia/Tokyo", hour:"numeric", hour12:false }).formatToParts(new Date());
...
function seedFromUUID(id: string){ return (parseInt(id.replace(/-/g,"").slice(0,8),16)>>>0); }
...
export async function GET(req: Request) {
  ...
  try {
    const r = await fetch(`${origin}/api/theme`,{ cache:"no-store", headers:{ cookie: cookieHeader }});
  ...
  const slot: Slot = (qs==="morning"||qs==="noon"||qs==="night") ? (qs as Slot) : getJstSlot();
  ...
  try {
    ...
    const messages = [
      ...
      { role:"user", content: JSON.stringify({ slot, theme, instruction:"ルネアとして1問4択の質問を生成してください。質問は50〜100文字以内、選択肢は20〜50文字以内で、すべて日本語で自然に。" })},
    ];
    const res = await client.chat.completions.create({ model:"gpt-5-mini", messages, temperature:0.7, response_format:{ type:"json_object" }});
    const raw = res.choices[0]?.message?.content ?? "{}";
    const json = JSON.parse(raw);
    text = json.question ?? "";
    options = Array.isArray(json.options) ? json.options : [];
  } catch {
    text = slot==="morning" ? "朝、最初の一歩として今のあなたに合うのはどれ？"
         : slot==="noon"    ? "昼、このあと数時間の過ごし方として近いのはどれ？"
                            : "夜、今日はどんな締めくくりが心地いい？";
    options = FALLBACK[slot];
  }

  const keys: EV[] = ["E","V","Λ","Ǝ"];
  let choices = (options||[]).slice(0,need).map((o,i)=>({
    key: (keys.includes(o?.key as EV) ? o!.key : keys[i]) as EV,
    label: String(o?.label ?? "").trim()
  })).filter(c=>c.label);
  const have = new Set(choices.map(c=>c.key));
  for (const c of FALLBACK[slot]) { if (choices.length>=need) break; if (!have.has(c.key)) { choices.push(c); have.add(c.key); } }
  ...
  const seed = seedFromUUID(question_id);
  const created_at = new Date().toISOString();
  const question = (text?.trim() ||
    (slot==="morning" ? "今のあなたに必要な最初の一歩はどれ？"
     : slot==="noon" ? "このあと数時間で進めたい進路は？"
                      : "今日はどんな締めくくりが心地いい？")) as string;

  const jar = cookies();
  jar.set("daily_question_id", question_id, { httpOnly:true, sameSite:"lax", path:"/" });
  jar.set("daily_slot", slot, { httpOnly:true, sameSite:"lax", path:"/" });
  jar.set("daily_theme", theme, { httpOnly:true, sameSite:"lax", path:"/" });

  return NextResponse.json({
    ok:true, question_id, seed, slot, theme,
    question: question.slice(0,100),
    choices: choices.map(c=>({ id:c.key, label:c.label })),
    created_at, env:"prod", _proxied:false
